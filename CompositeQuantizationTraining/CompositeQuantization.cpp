	...
	const bool initial)
{
	...
	GetDictionaryCrossProducts(dictionary_);
	GetDistortionsConstants();
...
void CompositeQuantization::GetBinaryCodes(
	const int iters,
	const PointType* points,
	const DictionaryType* dictionary,
	CodeType* binary_codes,
	const string binary_codes_output_file)
{
	mu_ = 0;
	InitDictionary(dictionary, dictionaries_count_, words_count_);
	GetDictionaryCrossProducts(dictionary_);
	GetDistortionsConstants();
	for (int iter = 0; iter < iters; ++iter)
	{
	...
	SaveBinaryCodes(binary_codes_output_file);
}
